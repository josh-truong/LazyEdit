...
int LazyEdit::decode_packet(AVCodecContext *dec, const AVPacket *pkt)
{
    ...
    while (ret >= 0) {
        ...
        if (dec->codec->type == AVMEDIA_TYPE_VIDEO) {
            ...
            uint8_t* frame_data;
            if (posix_memalign((void**)&frame_data, ALIGNMENT, width * height * 4) != 0) {
                printf("Couldn't allocate frame buffer\n");
                return 1;
            }
            sws_scale_video_frame(frame, frame_data);
...
int LazyEdit::decode()
{
    ...
    while (av_read_frame(fmt_ctx, pkt) >= 0 && !glfwWindowShouldClose(window)) {
